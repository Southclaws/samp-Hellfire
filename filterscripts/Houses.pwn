...
// External Dependencies (For movement to main script)
...
#include "../scripts/Unused/HousesData.pwn"
...
PlayerBuyHouse(playerid, houseid)
{
	...
	else
	{
		...
		format(szLabel, 64, "%d, %s\nOwner: %s", HouseData[houseid][h_Number], HouseData[houseid][h_Street], gPlayerName[playerid]);
...
{
	if(!strcmp(gPlayerName[playerid], HouseData[houseid][h_Owner], true) && strlen(HouseData[houseid][h_Owner])>0)return 1;
	else return 0;
}
...
/*
	Editing, Saving, Loading and using Furniture!
	    ...
	    This feature hooks OnPlayerKeyStateChange by simple call (Change this
	    method if you please, I will probably implement y_hooks soon)
*/
enum EDIT_TYPE
{
	House,
