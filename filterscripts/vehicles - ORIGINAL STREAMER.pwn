...
new
	...
	gVehicleData[GRID_TOTAL+1][MAX_STREAMED_IN_AREA][E_VEHICLE_DATA],
...
/*
		...
		if(pass==0)gZoneCol[idx] = 0x606060AA;
		else gZoneCol[idx]  = 0x808080AA;
		pass =! pass;
*/
...
public OnFilterScriptExit()
{
	...
	{
	    for(new j; j < MAX_STREAMED_IN_AREA; j++)
	    {
...
OnPlayerChangeArea(playerid, newareaid, oldareaid)
{
	...
	tmpNewAreas[4] = newareaid-GRID_DIVIDE;

	if(-1 < tmpNewAreas[0] < GRID_TOTAL)
	{
		CreateAreaVehicles(newareaid, playerid);
		gPlayerCurrentArea[playerid] = newareaid;
	}
	if(-1 < tmpNewAreas[1] < GRID_TOTAL)	CreateAreaVehicles(tmpNewAreas[1], playerid);
	if(-1 < tmpNewAreas[2] < GRID_TOTAL)	CreateAreaVehicles(tmpNewAreas[2], playerid);
	if(-1 < tmpNewAreas[3] < GRID_TOTAL)	CreateAreaVehicles(tmpNewAreas[3], playerid);
	if(-1 < tmpNewAreas[4] < GRID_TOTAL)	CreateAreaVehicles(tmpNewAreas[4], playerid);
...
//	gPlayerCurrentArea[playerid] = GRID_TOTAL;
	...
	tmpOldAreas[4] = oldareaid-GRID_DIVIDE;
	...
	while(i<5)
	{
		...
		if(j==5)DestroyAreaVehicles(tmpOldAreas[i], newareaid, playerid);
		i++;
	}


//	DestroyAreaVehicles(GRID_TOTAL, newareaid);

	return 1;
}
CreateAreaVehicles(area, playerid)
{
...
DestroyAreaVehicles(area, newarea, playerid)
{
