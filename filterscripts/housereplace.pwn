...
public OnFilterScriptInit()
{
		...
		while (fread(file, line))
		{
			...
			if (ipl)
			{
				...
				while (fread(ipl, line))
				{
					...
					newmodel = GetSAMP03EModel(model);
					if (newmodel)
					{
						// Los Santos
						if (19505 <= newmodel <= 19511)
						{
							QuatToEulerZXY(qx, qy, qz, qw, rx, ry, rz);
							AddObject(newmodel, px, py, pz, rx, ry, rz, LS);
							continue;
						}
						// San Fierro
						if (19489 <= newmodel <= 19495)
						{
							if(model == 3833)
							{
								pz -= 1.3;
							...
							QuatToEulerZXY(qx, qy, qz, qw, rx, ry, rz);
							AddObject(newmodel, px, py, pz, rx, ry, rz, SF);
							continue;
						}
						// Las Venturas
						if (19497 <= newmodel <= 19503)
						{
							QuatToEulerZXY(qx, qy, qz, qw, rx, ry, rz);
							AddObject(newmodel, px, py, pz, rx, ry, rz, LV);
							continue;
						}
					}
				}
	...
	print("Completed House Conversion");
}
...
AddObject(model, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, File:handle)
{
	...
	if(model == 19495)
	{
		...
		SaveOffsetObj(6959, x, y, z, rx, ry, rz, -14.8616, 14.4149, -4.6739,   0.00, 0.00, 0.00, handle);
	}
	if(model == 19493)
	{
