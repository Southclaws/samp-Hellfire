...
PlayerBuyHouse(playerid, houseid)
{
	...
	if(pMoney < HouseData[houseid][h_Price])return 0xABAD1DEA;
	else
	{
...
PlayerEnterEditMode(playerid, houseid, roomid)
{
    ...
    PlayerEditing[playerid][Object]=CreateDynamicObject(2357, frnObjectPos[playerid][0], frnObjectPos[playerid][1], frnObjectPos[playerid][2], 0.0, 0.0, 0.0, houseid+VIRTUALWORLD_OFFSET, InteriorData[HouseData[houseid][h_IntID]][intID], playerid);
...
CheckArea(direction, playerid, houseid, roomid, Float:xMove, Float:yMove)
{
	...
	if(direction==1)
	{
	    ...
	    if(frnObjectPos[playerid][1]+xMove != frnObjectPos[playerid][1])
	    {
			...
			{
				frnObjectPos[playerid][0]=HouseRoomBounds[HouseData[houseid][h_IntID]][roomid][YMAX];
				return 0;
			}
		}
	}
